...
export const submitContact = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    // TODO (Lovable Emails): όταν ρυθμιστεί το email domain, στείλε notification
    // στον ιδιοκτήτη με sendTemplateEmail("contact-owner-notification", ...).
    // Προς το παρόν καταγράφουμε τη φόρμα στα server logs.
    console.log("[contact] νέο μήνυμα:", {
      name: data.name,
      phone: data.phone,
      email: data.email,
      area: data.area || null,
